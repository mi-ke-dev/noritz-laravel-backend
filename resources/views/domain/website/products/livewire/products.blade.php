<div class="w-row">
            ...
            <div>
                <div class="w-form">
                    ...
                    <form id="email-form" name="email-form" data-name="Email Form" method="get">
                        ...
                        {{-- <h4 class="filterh">Accessories</h4> --}}
                    </form>
    ...
    <div class="column w-col w-col-10">
        @if(empty($heater_type)  && empty($size) && empty($type) && empty($features) && empty($install_location)
        && empty($efficiency))
            @if(!empty($heater_series))
                <div class="div-block-6">
                    <div id="w-node-_20de7921-7567-cdd1-2c33-64433101adfb-58d34ade" class="res">
                        <h2 class="heading-12">{{$heater_series->name}}</h2>
                    </div>
                ...
                <div class="div-block-9">
                    <div class="w-layout-grid grid-11">
                        @if(!empty($heater_series->heaters))
                            @foreach($heater_series->heaters as $heater)
                                <a href="{{ $ifremUrl }}/products/{{Str::slug($heater->model_no)}}" target="_parent">
                                    <div>
                                        <img src="{{Storage::url($heater->series_image)}}" loading="lazy" sizes="100vw" class="image-8" onerror="this.src='{{asset('images/nrcr.jpg')}}'">
                                        <div>
                                            <h4 class="tnames">{{$heater->model_no}}</h4>
                                        </div>
                                    </div>
                                </a>
                            @endforeach
            ...
            @else
                @if(!empty($residental_heaters) && count($residental_heaters)>0)
                    <div class="div-block-6">
                        <div id="w-node-_20de7921-7567-cdd1-2c33-64433101adfb-58d34ade" class="res">
                    ...
                    <div class="div-block-9">
                        <div class="w-layout-grid grid-11">
                            @if(!empty($residental_heaters))
                                @foreach($residental_heaters as $residental_heater)
                                    @if(isset($residental_heater->name))
                                        <a href="javascript:void(0);" wire:click="heater_series('{{$residental_heater->id}}')">
                                            <div>
                                                <img src="{{Storage::url($residental_heater->thumbnail_store_path)}}" loading="lazy" sizes="100vw" alt=""
                                                    class="image-8" onerror="this.src='{{asset('images/nrcr.jpg')}}'">
                                                <div>
                                                    <h4 class="tnames">{{$residental_heater->name}}</h4>
                                                </div>
                                            </div>
                                        </a>
                                    @endif
                ...
                @if(!empty($commercial_heaters) && count($commercial_heaters)>0)
                    <div class="div-block-6">
                    ...
                    <div class="div-block-9">
                        <div class="w-layout-grid grid-11">
                            @if(!empty($commercial_heaters))
                            @foreach($commercial_heaters as $commercial_heater)
                            @if(isset($commercial_heater->name))
                               <a href="javascript:void(0);" wire:click="heater_series('{{$commercial_heater->id}}')">
                            <div>
                                <img src="{{Storage::url($commercial_heater->thumbnail_store_path)}}" loading="lazy" sizes="100vw" alt="" class="image-8" onerror="this.src='{{asset('images/nrcr.jpg')}}'">
                                <div>
                                    <h5 class="tnames">{{$commercial_heater->name}}</h5>
                                </div>
                            </div>
                                </a>
                            @endif
        ...
        <div class="div-block-9">
            @if(!empty($search_heaters) && count($search_heaters)>0)
            <div class="w-layout-grid grid-11">

                    @foreach($search_heaters as $search_heater)
                        @foreach($search_heater->heaters as $heater)
                        <a href="{{ $ifremUrl }}/products/{{Str::slug($heater->model_no)}}" target="_parent">
                        <div>
                            <img src="{{Storage::url($heater->series_image)}}" loading="lazy" sizes="100vw" alt=""
                                class="image-8" onerror="this.src='{{asset('images/nrcr.jpg')}}'">
                            <div>
                                <h4 class="tnames">{{$heater->model_no}}</h4>
                            </div>
