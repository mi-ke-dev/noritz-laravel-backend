<div class="w-container sizing_klc">
    ...
    <div class="w-row" id="step-1">
        <div class="w-col w-col-12 steps zip_code">
            ...
            <form wire:submit.prevent="stepOneSubmit">
                ...
                @enderror
                @if ( session()->has('invalidZipCode') )
                <p class="text-danger">{{ session('invalidZipCode') }}</p></br>
                @endif
                    ...
                    <div class="w-row">
                        <div class="w-col w-col-6">
                            <div class="form-grpup">
                                ...
                                <input class="input-number" name="noofShower" id="noofShower" type="text" value="{{$noofShower}}" min="0" max="100" wire:model="noofShower">
                                <button type="button" class="input-number-increment" wire:click="increment">+</button>
                            </div>
                            @if ( session()->has('info') )
                            <span class="text-danger">{{ session('info') }}</span>
                            @endif
                            @error('noofShower') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div class="w-col w-col-6">
                            ...
                            @error('bodySpray') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="w-col w-col-6">
                            <div class="form-grpup">
                            ...
                            @error('recirculation') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                        <div class="w-col w-col-6">
                            <div class="form-grpup">
                            ...
                            @error('replaceBoliler') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>
                    </div>
    ...
    <div class="w-row" id="step-3">
        <div class="w-col w-col-12 steps recommend_product">
            ...
            <h2>We Recommend</h2>
            <ul>
                {{$contactNotrizMessage}}
                @foreach ($heaters as $heater)
                <li>
                    <div class="pr_img">
                        <img src="{{asset($heater->FullMarketingImage)}}" alt="">
                    </div>
                    <div class="pr_info">
                        <h2>{{$heater->series}}</h2>
                        <p>
                            Max: {{$heater->max_gas_input_btu}} btuh<br>
                            Min: {{$heater->min_gas_input_btu}} btuh<br>
                            Max Flow:{{$heater->max_flow_rate}}<br>
                            Efficiency: {{$heater->thermal_efficiency}}
                        </p>
                        @if($heater->spec_sheet && $heater->spec_sheet !='')
                        <a class="Spec_Sheet" target="_blank" href="{{ $this->getDownloadFile('spec_sheet',$heater->model_no) }}">Download Spec Sheet</a>
                        @endif
                        <a class="GetaQuote" target="_blank" href="https://contractorfinder.noritz.com/contractors/postal/{{$zip_code}}">Get a Quote</a>
                    </div>
                </li>
                @endforeach
            </ul>
